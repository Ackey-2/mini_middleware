...
namespace {
NodeOptions options_with_shm(bool enable_shm) {
    NodeOptions options;
...
Node::Node(std::string name, bool enable_shm)
    : Node(std::move(name), options_with_shm(enable_shm)) {}
...
Node::Node(std::string name, const NodeOptions& options)
    ...
    discovery_ = std::make_unique<DiscoveryAgent>(name_, loc, options.discovery_group,
                                                  options.discovery_port);

    // 3. 告诉数据面本地身份(participant_id + host_id),用于同机/跨机选路。
    //    必须在第一个匹配回调前完成 —— 此时 discovery 尚未 start。
    data_plane_->set_local_identity(discovery_->participant_id(), local_host_id(),
                                    options.enable_shm);
    ...
    LOG_INFO("Node created: {} (data port {}, shm={})",
             name_, data_plane_->server_port(), options.enable_shm ? "on" : "off");
}
